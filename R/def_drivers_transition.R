#' Deforestation driver transition map
#'
#' Downloads MapBiomas rasters for two years, classifies transitions into
#' deforestation drivers using `def_drivers()`, and generates a map..
#' Optionally saves the transition raster (GeoTIFF) and a CSV of transition
#' areas (hectares).
#'
...
#' @examples
#' \dontrun{
#' def_drivers_transition(aoi, year1 = 2000, year2 = 2020)
#' }
#'
...
def_drivers_transition <- function(aoi, year1, year2,
                                   title = NULL,
                                   output_dir = NULL,
                                   write_raster = FALSE,
                                   write_csv = FALSE) {
  ...
  # Aggregate the transition IDs into groups with def_drivers function
  groups <- select(def_drivers(), c(transition_type, transition_id))
  ...
  legend_df <- def_drivers()[index_classes, ]
  ...
  if(is.null(title))
    title <- paste0("Deforestation Drivers: ", year1, " to ", year2) # default title
  ...
  if (write_raster) {
    ...
    raster_filename <- paste0(aoi_name, "_def_drivers_raster_",
                              year1, "_", year2)
  ...
  if (write_csv) {
    ...
    csv_filename <- paste0(aoi_name, "_def_drivers_",
                           year1, "_", year2, ".csv")
