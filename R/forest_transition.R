#' Forest loss, gain, and stability transition map
#'
#' Downloads MapBiomas rasters for two years, classifies transitions into
#' four categories (remained forest, forest loss, forest gain, non-applicable)
#' using `forest_related()`, and generates a map. Optionally saves
#' the transition raster (GeoTIFF) and a CSV of transition areas (hectares).
#'
...
forest_transition <- function(aoi, year1, year2,
                                      title = NULL,
                                      output_dir = NULL,
                                      write_raster = FALSE,
                                      write_csv = FALSE) {
  ...
  # Aggregate the transition IDs into groups with forest_related function
  groups <- select(forest_related(), c(transition_type, transition_id))
  ...
  legend_df <- forest_related()[index_classes, ]
  ...
  if(is.null(title))
    title <- paste0("Forest Related Transition: ", year1, " to ", year2) # default title
  ...
  if (write_raster) {
    ...
    raster_filename <- paste0(aoi_name, "_forest_transition_raster_",
                              year1, "_", year2)
  ...
  if (write_csv) {
    ...
    csv_filename <- paste0(aoi_name, "_forest_transition_",
                           year1, "_", year2, ".csv")
